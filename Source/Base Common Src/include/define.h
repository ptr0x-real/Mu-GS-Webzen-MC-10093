...
#define GAMESEVER_VERSION	"1.00.92"//GetGameServerVersion()			// 게임서버 버전
...
// 사내 테스트서버 설정
#ifdef _RTEST
	#define TESTSERVER	 		1					// Service:0  테스트 서버인가..

	#define LOCAL_DATATEST							// 각 국가별 데이타 폴더를 이용 체크섬&데이터 테스트를 한다.
#else
	#define TESTSERVER	 		0					// Service:0  테스트 서버인가..

	#define LOCAL_DATATEST							// 각 국가별 데이타 폴더를 이용 체크섬&데이터 테스트를 한다.

//	#ifdef FOR_CHECKSUM_SERVER			// 게임서버 인증을 안하고 로컬 데이타폴더를 이용
//		#define LOCAL_DATATEST
//	#else	// FOR_CHECKSUM_SERVER		// 게임서버 인증을 사용
// 게임서버 인증 (foguy)
...
// 공성서버 설정
#ifdef _CS
	#define GAMESERVER_TO_CASTLESIEGESERVER			// 공성전 전용서버로 변경
	#define GAMESERVER_TO_CRYWOLFSERVER				// 크라이울프 전용서버로 변경
#endif
...
//국가별 NProtect버전 설정
	...
	// ** 일본 컴파일 시 게임가드 2.0버전 모듈을 프로젝트에 추가한 상태로 컴파일을 해야 함.
	// 추가 파일 : ggsrvdll.lib, ggsrvlib.lib
	...
	#define NPROTECT_GAME_GAURD_FOR_SERVER_VERSION_20_20050411
#else
	// 클라이언트 인증 서버용 라이브러리 1.5 사용 (u4us)
	#define NPROTECT_GAME_GAURD_FOR_SERVER_VERSION_15_20050411
	// 클라이언트 인증 서버용 라이브러리 1.0 사용 (u4us)
	#define NPROTECT_GAME_GAURD_FOR_SERVER_VERSION_10_20050411
#endif
...
#define FOR_NEW_TESTSERVER						// 테스트(본섭) 서버용...
#define FOR_NEW_LIVESEVER						// 신섭(본섭) 서버용...

#define FOR_CHECKSUM_SERVER						// 체크섬 서버에서만사용 (GSAuth 사용안함, 로컬드라이브 파일 사용)

// #define FOR_TEMP_SERVER							// 체험서버 설정시에만 사용한다. 임시 PC방 포인트 테이블에 설정된다. 이후 FOR_NEW_TESTSERVER와 정리가 필요할 것이다.

#ifdef FOR_TEMP_SERVER
	//[업데이트] 시즌 4 업데이트(***NPC 체험 서버용***) - 스탯 초기화 (grooving)
	#define EXPERIENCE_SERVER_NPC_STAT_RESET
#endif 

// 방송용
//#define FOR_ONAIR								// 방송용 (서버메시지 출력 않됨)
...
// 쓰면 안되는 디파인들
//#define LOCAL_DATATEST							// 각 국가별 데이타 폴더를 이용 체크섬&데이터 테스트를 한다.
//#define CHECKSUM_REMOVE							// 체크섬 체크를 제외한다 -> 2003.09.10 클라이언트 패치를 위함
//#define PRINT_MAGICBOOK_ITEMINSERT_LOG		// 법서의 아이템 드롭율을 로그파일에 출력
//#define MAKE_ITEM_STOP						// "/만들기" 명령 금지 -> 테스트 서버에서도
//#define CS_MAKE_ITEM_POTION					// 축복의물약, 영혼의물약, 라이프스톤외의 아이템은 만들 수 없음 (/만들기 않됨)
//#define MUTE_EVENT_MESSAGE					// 공지 막기
#define ADD_MINIDUMP_20081222					// 덤프파일 생성 한다.(grooving)
#define MUTEX_REMOVE							// 뮤텍스 체크를 제외한다
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
#if 0
//#define CHECK_CRT_DBG (happycat) - 가능하면 하지 말것!

// ##UPDATE-SET-LEVEL400-20041207 (apple)
//#define FOR_CASTLE_TESTSERVER					// 캐슬 테스트 서버용..
...
#define	WEBZEN_PUBLIC_IPADDRESS	"218.234.76.254"	// 한국 공인 IP주소 (b4nfter)

#ifdef FOR_KOREA
	// [버그수정] PC방 인증 대기 중 접속을 끊는 경우 사용자 삭제 처리가 가능하도록 수정 (hnine)
	...
	// [수치조정] PC방 포인트 시스템 프로모션 - PC방 포인트 시간 조절(RKFKA) 20070821 종료
	//#define MODIFY_PCBANG_POINT_TIME_EVENT_20070709
	// [업데이트] PC방 포인트 시스템 (RKFKA)
#if GAME_VERSION >= G_V_S4_5
	#define PCBANG_POINT_SYSTEM_20070206

	// [버그수정] 사용자 과금 남은시간 출력 관련 버그 수정 (hnine)
	#define MODIFY_BILLING_RESTTIME_BUGFIX_20070212

	// [버그수정] 과금 메시지가 3분마다 출력되는 버그 수정 (hnine)
	#define MODIFY_BILLING_MESSAGE_BUGFIX_20070208

	// [버그수정] PC방 통합빌링 이후 과금 만료 접속자에 대한 PC방 해택 삭제
	//#define MODIFY_PCBANG_EXPIRE_USER_ADVANTAGE_REMOVE_20070120

	// PC방 통합빌링, PC방 사용자 확인과 PC방 정량 남은시간에 대한 수정
	#define PCBANG_UNIFICATION_BILLING_CONNECT_USER_COUNT_20070103

	// PC방 유저 확인과, PC방 기본 혜택을 분리한다. (apple_hnine)
	#define PCBANG_ADVANTAGE_EXPAND_20060919

	// << 20060918 PC방 유저 설정 디파인,변수 관련 코멘트 (apple_hnine) >>
	// 
	// Q) 기본 PC방 혜택이란? - 체력상승, 150랩이하 추가경험치, 칼리마무료이동, 추가젠, 사탄수호등 내구도혜택
	//
	// D E F I N E  
	//		[PCBANG_ADVANTAGE] - PC방 유저인지를 인증서버를 통해 받음. 기본 PC방 혜택 적용
	//		[PCBANG_COUPON_EVENT_20060124] - PC방 쿠폰이벤트로 PCBANG_FREE_KALIMA_EVENT_20060124과 같이 적용되야 함.
	//		[PCBANG_FREE_KALIMA_EVENT_20060124] - PC방 유저인지를 구분해 칼리마 무료 이동 시킴. 기본 PC방 혜택 없음
	//
	// V A R I A B. 
	//		m_bPCBangUser, m_bPCBangCouponUser
	//
	// D E S C R I.
	//		m_bPCBangUser, m_bPCBangCouponUser 두 변수 모두 PC방 유저인지를 나타낸다. (20060918 수정함)
	//		
	//		PCBANG_ADVANTAGE 의 경우 인증서버에서 유저정보를 받는 함수와 맵서버 이동 함수의 확장을 통해 PC방 유저인지를 확인한다.
	//		주의!!> PC방 유저 구분 시 반드시 사용되어야 한다.
	//		주의!!> 인증 서버와의 프로토콜 처리 함수가 확장 되었으므로 PC방 유저인지는 구분하고 각종 PC방 혜택을 주고 싶지 않을 경우에도
	//				m_PCBangUser 는  FALSE로 하면 안된다. 맵 이동 시 이 변수 값을 통해 PC방 유저인지를 체크하기 때문.
	//
	//		PCBANG_FREE_KALIMA_EVENT_20060124 와 PCBANG_COUPON_EVENT_20060124 은 PCBANG_ADVANTAGE 에서 구현한 각종 PC방 혜택을 사용하지 않는다.
	//		주의!!> m_bPCBangUser 로 혜택을 주는 부분 이전에 if에 FALSE 설정을 함으로써 PC방 혜택이 작동 되지 않게 함.
	//		주의!!> PC방 기능을 키고 PC방 기본 혜택을 적용하고 싶지 않으면 PCBANG_ADVANTAGE, PCBANG_FREE_KALIMA_EVENT_20060124 를 켠다.
	//		주의!!> PC방 쿠폰기능까지 적용하려면 PCBANG_ADVANTAGE, PCBANG_FREE_KALIMA_EVENT_20060124, PCBANG_COUPON_EVENT_20060124 를 켠다.

	#define			PCBANG_ADVANTAGE		// happycat@20050201 (PC방 혜택)

	#define			HAPPY_POUCH				// happycat@20050201 (복주머니) - 국내 설 이벤트
	#define			HAPPY_CAT
	#define			BUGFIX_CAT
...
#endif
	// 블러드 캐슬용 NPC대화 ON/OFF
	//#define BLOOCASTLE_NPC_TALK_20031215

#if GAME_VERSION >= G_V_S2_2
	// 인증에서 사용자 인증할때 PC방 Guid포함 (블러드캐슬 2차이벤트(PC방) 작업을 위해
	#define ADD_PROTOCOL_PCBANG_GUID

	// 피씨방 요금 미납자 처리를 위한 디파인 
	#define PCBANG_DEFAULTER
	// 피씨방 요금 미납자 처리를 위한 디파인 끝
#endif
	// 채팅 로그 기록을 위한 디파인
	//#define CHAT_LOG
	// 채팅 로그 기록을 위한 디파인 끝
	...
	// 내구도 감소 보류
	//#define DURABILITY_DOWN_CHANGE					// 내구도 감소 방법 변경
	//#define REPAIR_ITEM_CHANGE						// 아이템 수리 변경	

	// 몬스터 아이템 드롭 로그파일 추가 (b4nfter)
	#define MONSTER_ITEMDROP_LOG_20040309		
#if GAME_VERSION >= G_V_99B
	// 블러드캐슬 하루 입장수 제한 (b4nfter)
	...
	// 해외 PC방 신규컨텐츠 적용(PC방 포인트 적용) 2008-04-01 by riverstyx
	#define ADD_FOREIGN_PCBANG_ADVANTAGE
	#ifdef ADD_FOREIGN_PCBANG_ADVANTAGE

		// [업데이트] PC방 강함의 인장  2008-04-01 by riverstyx
...
//	#define JAPAN_CHAOSMIX_BUG_20040701					// 일본 카오스조합 버그수정	-> 한국 이미 수정 됨

	#define JAPAN_DESTROYKING_EVENT_20040625			// 일본 파괴왕 이벤트

//	#define JAPAN_NPC_NEWTALK							// 일본 NPC새로운 말들..
	#define MODIFY_JAPAN_MONSTER_VALUE					// 일본 몬스터 수치 조정

	#define JAPAN_ROCFEATHER_CBMIX_BLOCK_20040303		// 일본 일반 카오스 조합 시 로크의 깃털이 들어가면 조합 않되도록

	#define JAPAN_1ST_ANIVERSARY_BOX_20040531			// 일본 1주년 기념 폭죽 축제
	...
	// [버그수정] 중국 접속 종료 차단 해킹 수정 (hnine)
	#define MODIFY_DENY_USER_DISCONNNECT_HACK_20080323

	// [버그수정] 중국 피로도 시스템 누적시간 버그 수정 (hnine)
	#define MODIFY_CHN_PLAYTIME_LIMIT_SYSTEM_02_20080218

	// [업데이트] 3차날개 조합 확률 수정 - 중국 전용 (hnine)
	#define MODIFY_THIRD_WING_MIX_RATE_20080116

	// 중국 PACKET_POSITION 프로토콜 코드 변경 (hnine)
	#define MODIFY_PACKET_POSITION_PROTOCOL_CODE_20071126

	// [업데이트] 중국 피로도 시스템 확장 (hnine)
	#define MODIFY_CHN_PLAYTIME_LIMIT_SYSTEM_01_20070509

	// 해외시즌1차 패치 : 중국 피로도 시스템 메시지 출력을 g_bChnPlayTimeLimitOn 옵션에 의해 On/Off 한다
	#define MODIFY_PLAYTIME_LIMIT_MESSAGE_ONOFF_20060427

	// 해외시즌1차 패치 : 중국에만 뮤택스 리무브를 한다.
	#define MUTEX_REMOVE

	#define	CHN_PLAYTIME_LIMIT_SYSTEM_20050818			// 중국 피로도 시스템 관련 (b4nfter)
	#define MODIFY_HACKLOG_OF_ATTACK_20050714			// 공격 핵 로그 수정 (b4nfter)
	#define MODIFY_HACKLOG_OF_ATTACK_20050518           // 공격 핵 로그 수정 (u4us)
//	#define MU_CHN_HACKLOG3_MODIFY_20041117             // 공격 핵 로그 수정 (foguy)
//	#define MU_CHN_HACKLOG_MODIFY_20040930				// 핵로그 저장 변경
	#define MU_CHN_OFFLINE_GIFT_EVENT_20040908			// 오프라인 경품 이벤트 행사 (몹 죽이면 경품 받음)
	#define MU_CHN_THANKS_COOKIE_EVENT_20040908			// 중국 고객감사 월병 이벤트
	#define MU_CHN_THANKS_RING_EVENT_20040908			// 중국 고객감사 영예의 반지 이벤트	
...
#ifdef FOR_TAIWAN
	//샵서버 재접 실패 수정(riverstyx)
	//#define MODIFY_RECONNECTSHOPSERVER_20080807

	// 대만 GM 쿤둔 변신 이벤트 추가 (hnine)
	#define EVENT_SERVER_OPEN_COMMEMORATION_20050510

	// 대만 GM 쿤둔 변신 이벤트를 위한 GM명령어 추가
	#define MODIFY_CHN_GMEVENT_TO_TAIWAN_GM_EVENT_20070906

#ifdef _FREE
	// [업데이트] 대만 무료서버 설정 (hnine)
	#define MODIFY_TAIWAN_FREE_SERVER_SETTING_20080214
#endif // _FREE

	// 대만 PACKET_POSITION 프로토콜 코드 변경 (hnine)
	#define MODIFY_PACKET_POSITION_PROTOCOL_CODE_20071126

	// 20051206 대만 PC방 이벤트 작업 (apple)
	// defined(PCBANG_ADVANTAGE) && defined(FOR_TAIWAN) <- 이것도 사용했음
	// ADD_PROTOCOL_PCBANG_GUID 정의 되면 안되기 때문에 주의!

	// 해외시즌1차 패치 : 대만 전용 PC방 해택 적용
	#define PCBANG_ADVANTAGE
	// 해외시즌1차 패치 : 대만에 뮤텍스 리무브 옵션을 켜준다.
	#define MUTEX_REMOVE
#endif

#ifdef FOR_THAILAND
	// 메달 이벤트 시 떨어지는 젠 변경 (b4nfter)
	#define MODIFY_MEDAL_EVENT_ZEN_20050829
	// 맵서버이동 시 성인인증 정보 보냄 (b4nfter)
	#define MAP_SERVER_ADDINFO_ADULT_20050610
	// 신년 이벤트 금속 그릇 아이템 드롭시 2548 젠 드롭 (u4us)
//	#define EVENT_SONGKRAN_2005_20050316                           
	// 폭죽 이벤트 관련 아이템 드롭 처리 수정 (u4us)
	#define MODIFY_DROP_ITEM_OF_FIRE_CRACKER_EVENT_20050316        
	// 낮은 레벨 유저들을 위한 보너스 경험치 부여 (u4us)
...
#ifndef FOR_THAILAND		
	// 데빌스퀘어 인원 감소 
	#define DEC_MAX_DEVILSQUARE_OBJCOUNT	// 태국은 데빌스퀘어 입장인원이 20명이다.
#endif

#ifdef FOR_VIETNAM

	// 옵션 리로드 후 CS와 연결 끊어지는 문제 수정(musang33)
	#define MODIFY_FAIL_CONNECT_CS_AFTER_OPTIONRELOAD_20091008

	// 수호의 부족 착용 한 유저와 결투 진행 시 죽여도 결투 포인트 증가 안되는 문제 수정(musang33)
	#define MODIFY_DUAL_AFFECT_BUFFTYPE_CHARM_GUARDIAN_20091007

	// 아이템 보호부적 착용시에도 펜린의 스킬로인한 아이템 내구도 감소 되는 문제 수정(musang33)
	#define MODIFY_NOT_EFFECT_FENRIRSKILL_IF_CHARM_PROTECTITEM_20090924

	// 아이템 내구도에 따른 판매가격 적용 안되는 문제 수정(musang33)
	#define MODIFY_SELLITEM_WITHOUT_DURABILITY_20090914

	// 아키루스 맵, 칸투르 마야전에서 눈사람, 팬더 변신반지 착용 되는 문제 수정(musang33)
	#define MODIFY_WEARING_PENDA_SNOWMAN_IN_ICARUS_AND_CANTUR_20090914

	// 망토조합부적 사용후 1차 날개 조합 시 다크드 3차 망토 생성되는 문제 수정(musang33)
	#define MODIFY_WINGCHAM_DARKLOAD3RD_WING_20090914

	// 맵 이동시 캐릭터가 밀리는 현상 수정(pelero)
	#define MODIFY_MOVE_MAP_MOON_WALKING_BUGFIX_20090908

	// 피로도 시스템 (apple)		-- 통합 소스 추가
	#define VTM_PLAYTIME_LIMIT_SYSTEM_20060626

	// 메세지 전송시 문자열 길이 계산 버그 수정 (RKFKA)		-- 통합 소스 추가
	//#define MODIFY_VIETNAM_MEMCPY_BUG_20060502
	// ** 베트남 컴파일 시 게임가드 2.0버전 모듈을 프로젝트에 추가한 상태로 컴파일을 해야 함.
	// 추가 파일 : ggsrvdll.lib, ggsrvlib.lib

	// 베트남 프록시 핵 방지 기능 (apple)

	// message.wtf 와 명령어 처리 등에 유니코드화가 적용된다.
...
#define MAX_DBINVENTORY				1728	// (8x8x10)+120+(8x4x10) 데이터베이스 읽기&저장되는 인벤토리데이터 크기
#else

// CastleEventItemList.cpp: implementation of the CCastleEventItemList class.
//
//////////////////////////////////////////////////////////////////////
...
CCastleEventItemList g_CastleEventItemList;
...
BOOL CCastleEventItemList::Load(char* filename)
{
	...
	{
		LogAdd("¡Ú Castle EventItemList Load Fail!!");
		return FALSE;
	}	
...
BOOL CCastleEventItemList::CommandMakeItem(int aIndex, char* sCommand)
{
	BOOL	bFindItem = FALSE;
	for( int i = 0; i < MAX_CS_MAKEITEMLIST_COUNT; i++ )
	{
		if( bLoadItemName[i] )
		{
			...
			{
				bFindItem = TRUE;
				for( int j = 0; j < MAX_CS_MAKESUBITEMLIST_COUNT; j++ )
				{

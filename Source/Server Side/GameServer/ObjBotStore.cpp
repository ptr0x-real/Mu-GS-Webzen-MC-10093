...
BYTE ObjBotStore::CheckSpace(LPOBJECTSTRUCT lpObj, int type, BYTE * TempMap)
{
...
BYTE INVENTORY_SIZEDEV(int aIndex, bool Max)
{
	if (aIndex < OBJ_STARTUSERINDEX || aIndex > MAX_OBJECT)
	{
...
void ObjBotStore::AddItem(int bIndex,int botNum)
{
	...
	{
		int blank = this->CheckSpace(&gObj[bIndex],this->bot[botNum].storeItem[i].num,&gObj[bIndex].pInventoryMap[INVENTORY_SIZEDEV(bIndex,false)]);
		if(blank != 255)
		{

			blank += INVENTORY_SIZEDEV(bIndex,true) - INVETORY_WEAR_SIZE;
			...
			item.Convert(this->bot[botNum].storeItem[i].num,item.m_Option1,item.m_Option2,item.m_Option3,item.m_NewOption,item.m_SetOption,3);

			gObj[bIndex].pInventory[blank].m_Option1 = item.m_Option1;
			gObj[bIndex].pInventory[blank].m_Option2 = item.m_Option2;
			gObj[bIndex].pInventory[blank].m_Option3 = item.m_Option3;
		...
		for(int botNum=0;botNum<MAX_BOTSTORE;botNum++)
		{
			if(this->bot[botNum].Enabled == true)
			{
				...
				if(result >= 0)
				{
					...
					gObj[result].Class = this->bot[botNum].Class >> 4;
					...
					gObj[result].pInventory = new CItem[MAIN_INVENTORY_SIZE+PSHOP_SIZE];					
					gObj[result].Inventory1 = new CItem[MAIN_INVENTORY_SIZE+PSHOP_SIZE];
					gObj[result].InventoryMap1 = new BYTE[MAIN_INVENTORY_SIZE+PSHOP_SIZE];
					gObj[result].pInventoryMap = new BYTE[MAIN_INVENTORY_SIZE+PSHOP_SIZE];

					for (int i=0;i<(MAIN_INVENTORY_SIZE+PSHOP_SIZE);i++)
					{
						gObj[result].pInventory[i].Clear();
						gObj[result].Inventory1[i].Clear();
					}

					memset(&gObj[result].pInventoryMap[0], (BYTE)-1, MAIN_INVENTORY_SIZE+PSHOP_SIZE);	//108+64+64
					memset(&gObj[result].InventoryMap1[0], (BYTE)-1, MAIN_INVENTORY_SIZE+PSHOP_SIZE);	//108+64+64
					...
					for(int i=0;i<9;i++)
					{
						if(this->bot[botNum].body[i].num >= 0 && this->bot[botNum].body[i].Enabled == true)
						{
							...
							//item.m_JewelOfHarmonyOption = 0;
							//item.m_ItemOptionEx = 0;
							item.Convert(this->bot[botNum].body[i].num,item.m_Option1,item.m_Option2,item.m_Option3,0,0,3);

							gObj[result].pInventory[i].m_Option1 = item.m_Option1;
							gObj[result].pInventory[i].m_Option2 = item.m_Option2;
							gObj[result].pInventory[i].m_Option3 = item.m_Option3;
					...
					ChatSend(&gObj[result],"Nueva venta.");
					GCActionSend2(&gObj[result],AT_SALUTE1,result,result);
				}
			}
		}
	}
